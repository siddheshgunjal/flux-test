...
def index():
    ...
    return render_template(
        "index.html", server=get_server_name(), test_duration=TEST_DURATION_SECONDS
    )
...
def health():
    """Basic health check"""
    return jsonify(
        {"status": "Online", "server": get_server_name(), "version": "0.1.0"}
    )
...
def ping():
    ...
    return jsonify({"server": get_server_name(), "timestamp": time.time()})
